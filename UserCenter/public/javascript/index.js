...
$().ready(function () {
	...
	$("#username").val('hu1024151886');
	$("#id").val('10000000');
	$("#password,#passwordCheck").val('123456')
	$("#phone").val('15570155773')
	$('#email').val('1024151886@qq.com')
...
function subFunc() {
	// $("#username").trrigger('change');
	// $("#id").trrigger('change');
	// $("#password").trrigger('change');
	// $("#passwordCheck").trrigger('change');
	// $("#phone").trrigger('change');
	// $("#email").trrigger('change');
...
function checkemail() {
	var name = $(this).val();
	var patten = new RegExp(/^[0-9a-zA-Z_\-]+@(([a-zA-Z_\-])+\.)+[a-zA-Z]{2,4}$/);
	if (patten.test(name)) {
		$("#errorInfo").text("请输入信息");
		$("#errorInfo").attr("class","correct");
		$(this).attr("class", "green");
